...
Selenium + Beautiful Soup
       │
       ▼
Raw CSV ──► pandas cleaning ──► Cleaned CSV
                                      │
                                      ▼
                                  PostgreSQL
                                      │
...
| `availability` | `TEXT` | Current stock status |
...
The CSV files contain the seven source fields. `book_id` and `loaded_at` are generated by PostgreSQL when data is loaded through the governed SQL workflow.
...
\copy pagepulse.books_data (book_names, availability, ratings, prices, categories, book_urls, book_images) FROM 'data/cleaned_data/books_data.csv' WITH (FORMAT CSV, HEADER TRUE, ENCODING 'UTF8')
```
...
Contains eight readable, business-oriented analyses:

1. Catalogue overview
2. Category assortment and performance
3. Highly rated premium products
4. Best-value books
5. Rating distribution
6. Price-band distribution
7. Categories that may need assortment improvement
8. Categories with availability risk
